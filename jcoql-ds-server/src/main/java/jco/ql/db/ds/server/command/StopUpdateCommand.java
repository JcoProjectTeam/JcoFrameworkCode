...
import jco.ql.db.ds.core.message.response.StopUpdateResponseMessage;
...
@JcoDsCommand
public class StopUpdateCommand extends AbstractCommand {
	
	private final DataSourceService dataSourceService;

	public StopUpdateCommand(DataSourceService dataSourceService) {
	...
	protected IMessageData doExecute(Properties serverSettings, Properties instanceMetadata, IMessageData request) {
		...
		return new StopUpdateResponseMessage(dataSourceService.stopUpdate(database,collection));
	}
