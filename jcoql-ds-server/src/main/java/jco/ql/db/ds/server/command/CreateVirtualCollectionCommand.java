...
import jco.ql.db.ds.core.message.response.CreateVirtualCollectionResponseMessage;
...
@JcoDsCommand
public class CreateVirtualCollectionCommand extends AbstractCommand {
	
	private final DataSourceService dataSourceService;

	public CreateVirtualCollectionCommand(DataSourceService dataSourceService) {
	...
	protected IMessageData doExecute(Properties serverSettings, Properties instanceMetadata, IMessageData request) {
		...
		if(DataSourceUtils.validCollectionName(name)) {
			...
			return new CreateVirtualCollectionResponseMessage(dataSourceService.createVirtualCollection(database, name, urls));
		} else {
